...
class CommonApiClient {
  ...
  fetchAllNotifications = (filters, userRole) => {
    if (userRole === ROLES?.MEMBER) {
      return axiosPrivate.get(
        `${API_URL.NOTIFICATION.MEMBER_NOTIFICATION}` +
          setNotificationQueryParams(filters)
      );
    } else {
      return axiosPrivate.get(
        `${API_URL.NOTIFICATION.AGENCY_NOTIFICATION}` +
          setNotificationQueryParams(filters)
      );
    }
  };
  ...
  fetchChannelList = () => axiosPrivate.get(API_URL.COMPANIES.CHANNEL);
}
