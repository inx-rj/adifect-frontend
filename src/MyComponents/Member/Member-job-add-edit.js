...
import { memberAdminWorkflowAction } from "../../redux/actions/Member-Admin-Workflow-Actions";
import { MemberAdminGetJobDetailsAction } from "../../redux/actions/Member-Admin-Job-List-Actions";
import { MEMBER_ADMIN_GET_JOB_DETAILS_RESET } from "../../constants/Member-Admin-job-list-constants";
...
export default function Member_job_add_edit() {
  ...
  const { memberAdmWorkf } = useSelector(
    (state) => state.memberAdminWorkflowReducer
  );
  ...
  // Get JobType selection from router state
  useEffect(() => {
    if (location.pathname === "/jobs/add" && (location.state?.['selectedJobType'] !== CREATE_JOB_TYPES.MEDIA && !location.state?.['selectedJobType'])) {
      // console.log({ jt: location.state?.['selectedJobType'], location });
      navigate('/jobs/list');
    }
  }, [location?.['state']])
  ...
  useEffect(() => {
    if (headerCompany) {
      // dispatch(memberAdminWorkflowAction(headerCompany));
      dispatch(
        listAgencyInHouseUsers(headerCompany, { agency: agecyIdCompany })
      );
      const successTemplateApi = api
        .get(
          `${BACKEND_API_URL}members/members-job-template/?company=${headerCompany}`
        )
        .then((res) => {
          // gettemplate(e);
          settemplatevalue(res?.data?.data?.results || res?.data?.data);
        });
      const success = api
        .get(`${BACKEND_API_URL}members/workflow/?company=${headerCompany}`)
        .then((res) => {
          // console.log(res.data);
          setapivalue(res?.data?.data?.results || res?.data);
        });
    }
  }, [headerCompany]);
  ...
  // useEffect(() => {
  //   if(memberAdmWorkf){
  //     setapivalue(memberAdmWorkf);
  //   }
  // }, [memberAdmWorkf]);

  //COMPANY GET SHOW IN COMPANY SELECT
  const [companydataGet, setcompanydataGet] = useState();
  ...
  useEffect(() => {
    ...
    const success = axios
      ...
      .then((res) => {
        // console.log(res);
        setcompanydataGet(res.data?.data);
      })
      .catch((err) => { });
  }, []);
  ...
  useEffect(() => {
    if (headerCompany) {
      setMemberAdminId(headerCompany);
  ...
  useEffect(() => {
    if (dam === true) {
      // console.log(Collectionviewdata);
      setexistingmediafile1(Collectionviewdata);
      // setexistingmediafile1([...mediafile1, Collectionviewdata]);
      setisfiles(true);
    }
  }, [moveit]);
  ...
  const { agencyInHouseUsers, success: successAgencyInHouseUsers } =
    useSelector((state) => state.listAgencyInHouseUsersReducer);
  ...
  useEffect(() => {
    ...
    window.scrollTo(0, 0);
  }, [member_Admin_job_detail_success]);
  ...
  useEffect(() => {
    dispatch({ type: MEMBER_ADMIN_GET_JOB_DETAILS_RESET });
  }, []);
  ...
  useEffect(() => {
    if (jobId) {
      ...
      // setShowdraft(false);
      settextchange(true);
      dispatch(MemberAdminGetJobDetailsAction(jobId));
      if (member_Admin_job_detail_success) {
        dispatch(listAgencyInHouseUsers(jobDetails.company));
        ...
        if (jobDetails.is_house_member) {
          ...
          setPrice(memberAdminJobDetails.price);
          setlevel(memberAdminJobDetails.level?.id);
          setJobType(memberAdminJobDetails.job_type);
        }

        if (memberAdminJobDetails.status != 0) {
          setShowdraft(false);
        }
        setTitle(memberAdminJobDetails.title);
        setDescription(memberAdminJobDetails.description);
        // setFiles(jobDetails.images);
        // setJobSampleDocuments(jobDetails.images)
        setDeliveryDate(new Date(memberAdminJobDetails.expected_delivery_date));
        setSkills(memberAdminJobDetails.skills);
        setindustryname(memberAdminJobDetails.industry);
        setdivid(memberAdminJobDetails.due_date_index);
        const success = api.get(`${BACKEND_API_URL}job-draft`).then((res) => {
          const success = api
            .get(
              `${BACKEND_API_URL}members/workflow/?company=${memberAdminJobDetails.company}`
            )
            .then((res) => {
              setapivalue(res.data?.data);
            });
        });
        setworkflowdata(memberAdminJobDetails.workflow);
        dispatch(getRelatedJobs(memberAdminJobDetails.company));
        setRelatedJobs(memberAdminJobDetails.related_jobs);
        setcompanyvalue(memberAdminJobDetails.company);
        setItemData(memberAdminJobDetails.jobtasks_job);
        setTemplatename(memberAdminJobDetails.template_name);

        for (
          let index = 0;
          index < memberAdminJobDetails.images.length;
          index++
        ) {
          if (
            memberAdminJobDetails.images[index].work_sample_image_name != null
          ) {
            setissamplefiles(true);
          }

          if (memberAdminJobDetails.images[index]?.job_image_name != null) {
            setisfiles(true);
          }
        }

        if (memberAdminJobDetails.related_jobs) {
          // setShow(true);
        }

        if (memberAdminJobDetails.jobtasks_job[0]) {
          setIsShowntask(true);
        }

        if (memberAdminJobDetails.image_url) {
          setisfiles(true);
          let newArray = memberAdminJobDetails.image_url.split(",");
          setFormUrls(newArray);
        }
        if (memberAdminJobDetails.sample_work_url) {
          setissamplefiles(true);
          let newArray = memberAdminJobDetails.sample_work_url.split(",");
          setFormSampleUrls(newArray);
        }
        if (memberAdminJobDetails.related_jobs?.length > 0) {
          setIsRelatedToPrevJob(true);
        }
        // setTags(jobDetails.tags);
        if (memberAdminJobDetails.tags) {
          const tagsList = memberAdminJobDetails.tags?.split(",");
          if (tagsList) {
            setTags(tagsList);
          }
        }
        if (memberAdminJobDetails.images) {
          let newImages = memberAdminJobDetails.images;
          newImages = newImages.filter((item) => item);
          setJobDocuments(newImages);
          let fileext = [];
          let s = [];
          let b = [];
          for (var i = 0; i < memberAdminJobDetails.images.length; i++) {
            fileext.push(
              memberAdminJobDetails.images[i].job_images?.slice(
                ((memberAdminJobDetails.images[i].job_images.lastIndexOf(".") -
                  1) >>>
                  0) +
                2
              )
            ...
            b.push(memberAdminJobDetails.images[i].job_images);
          }
          // console.log(s);
        ...
        if (memberAdminJobDetails.images) {
          ...
          for (var i = 0; i < memberAdminJobDetails.images.length; i++) {
            fileext1.push(
              memberAdminJobDetails.images[i].work_sample_images?.slice(
                ((memberAdminJobDetails.images[
                  i
            ...
            b1.push(memberAdminJobDetails.images[i].work_sample_images);
          }
          // console.log("s--", s);
      ...
      // const success = await api.get(`${BACKEND_API_URL}job-draft`).then((res) => {
      //   const success = api
      //     .get(
    ...
    setEditFileSampleExtension([]);
    setEditFileExtension([]);
  }, [member_Admin_job_detail_success, successAgencyInHouseUsers]);
  ...
  useEffect(() => {
    ...
    const industry = api
      ...
      .then((res) => {
        setindustryvalue(res?.data?.data?.results || res?.data?.data);
      });
  }, []);
  ...
  const validateSubmit = (e, data) => {
    ...
    const tempErrors = {
      ...
      price: !isBudgetNotRequired && validations.price(price),
      ...
      level: !isBudgetNotRequired && !level && "Please select an level",
  ...
  const submitHandler = async (data) => {
    ...
    formData.append('create_job_type', Number(location.state?.['selectedJobType'] ?? jobDetails?.create_job_type))
    ...
    formData.append("company", memberAdminId);
    ...
    formData.append("user", agecyIdCompany);
    // formData.append("company",header );
    ...
    if (jobId && data === "post" && !isShown) {
      const create_job = await axios
        .put(
          `${BACKEND_API_URL}members/member-jobs/${jobId}/`,
          formData,
          config
        )
        .then((res) => {
      ...
      const update_job = await axios
        .post(`${BACKEND_API_URL}members/member-jobs/`, formData, config)
        .then((res) => {
          ...
          if (data == "draft") {
            ...
            navigate(`/member-templates/list`);
          } else {
            navigate(`/jobs/list`);
          }
        })
  ...
  const getvalue = (e) => {
    ...
    const success = api
      .get(`${BACKEND_API_URL}agency/company/${e.target.value}`)
      .then((res) => {
    ...
    if (e.target.value !== null) {
      dispatch(listAgencyInHouseUsers(e.target.value));
  ...
  const gettemplate = (e) => {
    const success = api
      .get(`${BACKEND_API_URL}members/members-job-template/${e.target.value}/`)
      .then((res) => {
            ...
            <div className="Describe half-form-validation ">
              <div className="formjobsagencyadd">
                <div className="job_details_content">
                  <h1 className="JobDetailsTitle">Job Details - {jobDetailsTitle(Number(location.state?.['selectedJobType'] ?? jobDetails?.create_job_type))}</h1>
                </div>
                <div
                  className={
                    errors.title
              ...
              {!(["Text Copy"].includes(jobDetailsTitle(Number(location.state?.['selectedJobType'] ?? jobDetails?.create_job_type)))) && (
                <>
