...
import {
  ...
  Damcopypost,
  InhouseDamCollectionSearch,
  InhouseCollectionfilter,
  getdamCollectionDetailswithid,
  InhouseCountfilter 
} from "../../redux/actions/Member-Dam-Actions";

function TableViewMember_All_Collection() {
  ...
  const movefilelocation = () => {
    ...
    setTimeout(() => {
      dispatch(getdamCollectionDetailswithid(collectionid));
    }, 1800);
  ...
  const favouritefile = () => {
    ...
    dispatch(Titleupdate(formData, folderdeleteid));
    setisfavouritefolder();
    setTimeout(() => {
      dispatch(InhouseCountfilter(collectionid,agecyIdCompany));
    }, 500);
    setdetailsid()
  };
    ...
    swal({
      ...
      if (willDelete) {
        if (DamDataCollectionId[0].dam_media.length <= 1) {
          ...
          if (count.length > 0) {
            dispatch(DamMutipledelete(count));
            setIsLoading(true);
          } else {
  ...
  const { CollectionDamFilter } = useSelector(
    (state) => state.InhouseCollectionDamFilter
  );

  const { CollectionDamCount } = useSelector(
    (state) => state.InhouseCollectionDamCount
  );

  const { CollectionDamSearch } = useSelector(
    (state) => state.InhouseCollectionDamSearch
  );
  ...
  const getvalue1 = (e) => {
    ...
    if (e.target.value == null) {
      ...
      dispatch(InhouseDamCollectionSearch(collectionid,e.target.value));
...
const [total, settotal] = useState([]);
  ...
  dispatch(InhouseCollectionfilter(collectionid,params))
  ...
  const handleFileSubmit = () => {
    ...
    if (inputRef.current.length) {
      ...
      indexvalue.current = indexvalue.current + 1;
        ...
        axios
          .post(`${BACKEND_API_URL}members/member-dam-media/`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
          ...
          .then((response) => {
...

            {
              ...
              let collectionid = localStorage.getItem("collectionid");
    dispatch(getdamCollectionDetailswithid(collectionid));
    dispatch(InhouseCountfilter(collectionid));
            }
          });
        ...
        <>
          ...
          <div className="upperclasscoll">
            <div className="upperclass">
              ...
              <div className="Createmidebtn btncreatesec">
                    <Link to="#">
                      <Button
                        id="basic-button"
            ...
            {parentid && (
              ...
              <div className="main_slider">
                ...
                <div className="common-datatable-slider">
              <div className="dataTableMediaDiv mediatableviewsec">
              ...
              <div className="clmediasec">
                <div className="damdatacolll">
                  {!searchfile && isfiles  && <>
                    {DamDataCollectionId?.map((item) => (
                    <>
...
{CollectionDamSearch?.videos && <>
                  ...
                  {isOpenFullVideo && (
                    <div className="fullVideoPlaydivVodeo">
                      ...
                      <div className="fullVideoCloseTagDiv mideatableicon1">
                ...
                <div className="collectiondataimg collectdatanew1">
                  ...
                  <div
                    className={
                      isDisplayDetails
                        ? "activity_main showMediaInfoDetails activity_main2 table11"
                        : "activity_main closeMediaInfoDetails activity_main2 table11"
                    }
                  >
                    <div
        ...
        <DialogTitle className="profileImgfolder imgsizefixer imgsizefixermediafolder">
          <h2>
            ...
            {!parentid && (
              <>
                {" "}
                <Link to={`/member-media`} className="closebuttonsec">
                  <i
...
export default TableViewMember_All_Collection;
