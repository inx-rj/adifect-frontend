...
import { DUPLICATE_MEMBER_ADMIN_JOB_LIST_IN_REVIEW_RESET } from "../../constants/Member-Admin-job-list-constants";
import { DuplicateMemberAdminJobListInReviewAction } from "../../redux/actions/Member-Admin-Job-List-Actions";

function Member_Dashboard_Admin_In_Review() {
  ...
  const {
    loading: loadingDuplicateMemberAdminJobList,
    duplicateMemberAdminJobList,
  } = useSelector((state) => state.MemberAdminGetJobInReviewListReducer);

  useEffect(() => {
    dispatch({ type: DUPLICATE_MEMBER_ADMIN_JOB_LIST_IN_REVIEW_RESET });
    if(headerCompany){
    dispatch(
      DuplicateMemberAdminJobListInReviewAction({
        id:headerCompany,
        status: 3,
        page: 1,
  ...
  return (
    <>
      {loadingDuplicateMemberAdminJobList ? (
        ...
        <div className="Work-In">
          <div className="Work-In ReviewIn">
            ...
            {duplicateMemberAdminJobList?.slice(0, 5)?.map((item) => (
              <>
            ...
            {duplicateMemberAdminJobList?.length > 5 && (
              <Link to={`/projects`} onClick={setLocalVarRedirect}>
                <span className="spanDashboardViewMore">View More...</span>
              </Link>
            )}
            {duplicateMemberAdminJobList?.length < 1 && (
...
export default Member_Dashboard_Admin_In_Review;
