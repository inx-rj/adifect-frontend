...
import { getTemplateDetails } from "../../redux/actions/jobTemplate-action";
import $ from "jquery";
...
import { MemberAdminTemplateUpdate } from "../../redux/actions/member-Admin-template-Action";
...
export default function Member_Admin_template_Edit() {
  const { MembertemplateId } = useParams();
  ...
  const {
    memberTemplateListUpdate,
    success: successUpdate,
    error,
  } = useSelector((state) => state.MemberAdminTemplateUpdateReducer);
  ...
  // useEffect(()=>{
  //   dispatch(MemberAdminTemplateUpdate(MembertemplateId))
  // },[successUpdate])
  ...
  const {
    ...
    success: dataupdate,
    templateDetails,
  } = useSelector((state) => state.templateDetailsReducer);
    ...
    if (headerCompany) {
      ...
      const success = api
        ...
        .then((res) => {
          ...
          setapivalue(res?.data?.data?.results || res?.data?.data);
        });
    }
  }, [headerCompany]);
  ...
  useEffect(() => {
    if (MembertemplateId) {
      setShow(true);
      settextchange(true);
      dispatch(getTemplateDetails(MembertemplateId));
      if (dataupdate) {
        if (templateDetails?.status != 0) {
          setShowdraft(false);
        }
        // alert(templateDetails?.description)
        setTitle(templateDetails?.title);
        setDescription(templateDetails?.description);
        // setFiles(templateDetails?.images);
        // setJobSampleDocuments(templateDetails?.images)
        setDeliveryDate(templateDetails?.expected_delivery_date);
        setPrice(templateDetails?.price);
        if (templateDetails?.level) {
          setlevel(templateDetails?.level?.id);
        }
        setJobType(templateDetails?.job_type);
        setSkills(templateDetails?.skills);
        setindustryname(templateDetails?.industry);
        setdivid(templateDetails?.due_date_index);
        const success = api.get(`${BACKEND_API_URL}job-draft`).then((res) => {
          const success = api
            .get(
              `${BACKEND_API_URL}members/workflow/?company=${templateDetails?.company}`
            )
            .then((res) => {
              setapivalue(res.data);
            });
        });
        setworkflowdata(templateDetails?.workflow);
        dispatch(getRelatedJobs(templateDetails?.company));
        setRelatedJobs(templateDetails?.related_jobs);
        setcompanyvalue(templateDetails?.company);
        setcompanyName(templateDetails?.company_name);

        // setItemData(templateDetails?.jobtasks_job);
        setTemplatename(templateDetails?.template_name);
        if (templateDetails?.get_jobType_details == "Fixed") {
          setJobType("0");
        }
        if (templateDetails?.get_jobType_details == "Hourly") {
          setJobType("1");
        }
        // if (templateDetails?.images[0]?.work_sample_image_name) {
        //   setissamplefiles(true);
        // }

        // if (templateDetails?.images[0]?.job_image_name) {
        //   setisfiles(true);
        // }

        // if (templateDetails?.jobtasks_job[0]) {
        //   setIsShowntask(true);
        // }

        if (templateDetails?.image_url) {
          setisfiles(true);
          let newArray = templateDetails?.image_url.split(",");
          setFormUrls(newArray);
        }
        if (templateDetails?.sample_work_url) {
          setissamplefiles(true);
          let newArray = templateDetails?.sample_work_url.split(",");
          setFormSampleUrls(newArray);
        }
        if (templateDetails?.related_jobs?.length > 0) {
          setIsRelatedToPrevJob(true);
        }
        // setTags(templateDetails?.tags);
        if (templateDetails?.tags) {
          const tagsList = templateDetails?.tags?.split(",");
          if (tagsList) {
            setTags(tagsList);
          }
        }
        if (templateDetails?.images) {
          let newImages = templateDetails?.images;
          newImages = newImages.filter((item) => item);
          setJobDocuments(newImages);
          let fileext = [];
          let s = [];
          let b = [];
          for (var i = 0; i < templateDetails?.images.length; i++) {
            fileext.push(
              templateDetails?.images[i].job_images?.slice(
                ((templateDetails?.images[i].job_images.lastIndexOf(".") -
                  1) >>>
                  0) +
                  2
              )
            ...
            b.push(templateDetails?.images[i].job_images);
          }
        ...
        if (templateDetails?.images) {
          ...
          for (var i = 0; i < templateDetails?.images.length; i++) {
            fileext1.push(
              templateDetails?.images[i].work_sample_images?.slice(
                ((templateDetails?.images[i].work_sample_images?.lastIndexOf(
                  "."
                ) -
                  1) >>>
                  0) +
            ...
            b1.push(templateDetails?.images[i].work_sample_images);
          }
  ...
  const submitHandler = async (data) => {
    ...
    formData.append("job_template_id", MembertemplateId);
    ...
    // console.log('MembertemplateId == ', MembertemplateId);
    dispatch(MemberAdminTemplateUpdate(MembertemplateId, formData));
  ...
  useEffect(() => {
    if (successUpdate && render) {
      swal({
        title: "Successfully Complete",
        text: "Successfully Job Updated",
          ...
          <div className="ContentDiv jobaddformtop" onClick={removeSkillPopup}>
            <div className="Describe half-form-validation ">
              <div className="formjobsagencyadd">
                ...
                >
                  <h4 className="">Template Title</h4>
                  <input
                ...
                >
                  ...
                  <div className="styled-select Companyname Companyname_1">
                    ...
                    >
                      <MenuItem value={companyvalue}>{companyName}</MenuItem>
