...
import {
    memberAdminWorkflowAction,
    memberAdminWorkflowDeleteAction,
  } from "../../redux/actions/Member-Admin-Workflow-Actions";

export default function Member_admin_workflow_companyData() {
  ...
  const { success: deleteWorkSuccess } = useSelector(
    (state) => state.memberAdminWorkflowDeleteReducer
  );
...
//   }, [success]);
  ...
  useEffect(() => {
      ...
      dispatch(memberAdminWorkflowAction(companyid));
...
//   }, [success]);
      ...
      item.action = (
        <div style={{ display: "flex" }}>
          ...
          >
            ...
            <p className="editiconDelete">Edit</p>
          </Link>
          <div style={{ display: "flex" }}>
            <Button disabled={item.assigned_job} title="delete" className="deletebutt">
              {" "}
              {/* <img
                className="editicon"
                src="/img/delet.png"
              ...
              />{" "} */}
              <p style={{ color: item.assigned_job && "grey" }} onClick={() => deleteHandler(item.id)} className="editiconDelete">
                Delete
              </p>
            </Button>
          </div>
  ...
  const deleteHandler = (id) => {
    swal({
      ...
      if (willDelete) {
        dispatch(memberAdminWorkflowDeleteAction(id));
        swal({
      ...
      {isLoading ? (
        ...
        <>
          <div className="Topallpage AllPageHight Custompage">
            <div className="ContentDiv Categoriesdiv1 Agencyworkflowpagetop AgencyworkflowpagetopPaddingRest">
