...
export default function Agency_approval_workflow() {
    ...
    if (workflowDetails && workflowId) {
      if (workflowDetails.length > 0) {
        for (let i in workflowDetails) {
          ...
          if (
            workflowDetails[i]?.nudge_time &&
            workflowDetails[i]?.nudge_time?.length > 0
          ) {
            const nudgeTimeArr = workflowDetails[i]?.nudge_time?.split(",");
            nudgeTimeArr.pop();
          ...
          finalArr.push({
            ...
            isDeadline: workflowDetails[i].approval_time ? true : false,
            approval_time: workflowDetails[i].approval_time
              ? workflowDetails[i].approval_time
              : 1,
            isOpenDeadline: false,
            is_nudge: workflowDetails[i].is_nudge,
            nudgeOpen:
  ...
  useEffect(() => {
    ...
    const success = axios
      .get(`${BACKEND_API_URL}agency/company/?is_active=1`, config)
      .then((res) => {
        // console.log(res);
        setcompanydata(res?.data?.data?.results || res?.data?.data);
      })
      .catch((err) => {});
  }, []);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="right-sidebar-workflow">
            {showbutton ? (
              <div className="workflow-heading edittitlework">
                ...
                <div className="text-content ">
                  ...
                  <div className="styled-select Companyname">
                    <Select
                      className={
                        companyvalue === null
                          ? "selectinputcolor agency-workflow-company-selenium"
                          : "menuiteminputcolor agency-workflow-company-selenium"
                      }
                      disabled={!showbutton}
                      open={isOpen6}
                      onOpen={() => {
                                    ...
                                    <div className="Observersskillssec">
                                      <div className="skills-input-container">
                                        <Autocomplete
                                          ...
                                          options={
                                            workflowMemberApproversData?.length >
                                            0
                                    ...
                                    {item?.is_observer && (
                                      <>
                                        ...
                                        <div
                                          className={
                                            errors.observer
                                              ? "Observersskillssec error1 ObserversskillssecSecond"
                                              : "Observersskillssec ObserversskillssecSecond"
                                          }
                                        >
                                          <div className="skills-input-container">
                                            <Autocomplete
                                        ...
                                        <>
                                          <div className="Observersskillssec">
                                            <div className="skills-input-container">
                                              ...
                                              {/* {console.log({ item, nudge: item?.nudge_time }, 'item?.nudge_time')} */}
                                              {item?.nudge_time?.map(
                                                (nudge, nudge_index) => (
                                                  <>
