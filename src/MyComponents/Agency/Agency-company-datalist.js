...
import Agency_wokflow_companydata from "./Agency-wokflow-companydata";
...
const Agency_company_datalist = () => {
  ...
  useEffect(() => {
    if (companiesDec) {
      setCompanyName(companiesDec?.name);
      setCompanyDec(companiesDec?.description);
      setEmail(companiesDec?.company_email);
      setWebsiteData(companiesDec?.company_website);
      setPhoneNum(companiesDec?.company_phone_number);
      setindustryname(companiesDec?.industry);
    }
  }, [companiesDec]);

  useEffect(() => {
    // console.log("error c update -- ", companydecUpdateSucceess);
    if (companydecUpdateSucceess && renderPage) {
      swal({
    ...
    if (companydecUpdateError && renderPage) {
      swal({
        title: "",
        text: companydecUpdateError,
  ...
  }, [companydecUpdateSucceess, companydecUpdateError]);
  ...
  const submitHandler7 = () => {
    ...
    dispatch(updateCompanyDetails(companyid, formData));
    setRenderPage(true);
    setOpen4(false);
  };
          ...
          <div className="inlineprivateProfileContent">
            <div className="privateProfileContent">
              <div className="PhotoprofileDiv">
                {useradminDetails?.profile_img ? (
                  <div className="userprofileimg">
                    <img
                      src={companiesDec?.company_profile_img}
              ...
              <div className="privateProfileName">
                ...
                <p>
                  {companiesDec?.description == "null"
                    ? ""
                    : companiesDec?.description}
                </p>
...
export default Agency_company_datalist;
