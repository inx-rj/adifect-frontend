...
export default function Agency_jobs_add_edit() {
          ...
          for (var i = 0; i < res.data.images.length; i++) {
            fileext.push(
              res.data.images[i].job_images?.slice(
                ((res.data.images[i].job_images.lastIndexOf(".") - 1) >>> 0) + 2
              )
            );
            // console.log(fileext);
            s.push(
  ...
  const validateSubmit = (e, data) => {
    ...
    const tempErrors = {
      ...
      template: !templatename && isShown && "Please select an template",
    };
  ...
  const submitHandler = async (data) => {
    ...
    if (jobId) {
      ...
      const create_job = await axios
        .put(`${BACKEND_API_URL}jobs/${draftid}/`, formData, config)
        .then((res) => {
          swal({
            title: "Successfully Complete",
            text: "Successfully Created",
                ...
                <div className="text-content Skills Experiencenew_2">
                  ...
                  {isShowntask && (
                    <div className="container containerPad">
                      ...
                      <button className="adddataButton" onClick={handleSubmit}>
                ...
                <div className="switch_Agency">
                  <div className="Observers1">
                    ...
                    {!textchange ? (
                      ...
                      <h4>Save as New Template</h4>
                    )}
                  </div>
                </div>
                {isShown && (
                  <div className="istoggle_Shown">
                    ...
                    <input
                      ...
                      type="text"
                      placeholder="Banner Ad design Template"
                      value={templatename}
                      onChange={(e) => {
