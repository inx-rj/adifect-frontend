...
export default function Agency_approval_workflow() {
  ...
  const handleRemoveStage = (id, isDatabase) => {
    console.log("id--", id);
    if (stages.length > 1) {
  ...
  const submitHandler = async () => {
    let newArr = stages?.map((item, index) => {
      ...
      for (let i = 0; i < item?.observer?.length; i++) {
        ...
        // console.log("fffffff", item.observer[i].length > 0);
        myArr2.push(item.observer[i].id);
      }
      // console.log(item);
      console.log(
        `index ${index}`,
        item.observer.length == 0,
        !item.is_observer
      );
      // console.log(`index ${index}`, !item.is_observer[index]);
      if (item.is_observer && item.observer.length == 0) {
        console.log("ert");
    ...
    console.log("9999", Object.keys(errors).length);

    if (errors.observer == null) {
      if (workflowId) {
                                ...
                                >
                                  ...
                                  <div className="Approvers">
                                    ...
                                    <div className="related-jobs-check-box">
                                      ...
                                      >
                                        {" "}
                                        Everyone must approve job before it will
