...
export default function Agency_template_edit() {
  ...
  useEffect(() => {
    if (templateId) {
      ...
      if (success) {
        dispatch(listAgencyInHouseUsers(templateDetails.company));

        if (templateDetails.is_house_member) {
          setIsBudgetNotRequired(true);
          if (templateDetails?.house_member?.length > 0) {
            const houseMembersList = [];
            for (
              let index = 0;
              index < templateDetails?.house_member?.length;
              index++
            ) {
              houseMembersList.push(
                agencyInHouseUsers?.find(
                  (item) => item.id == templateDetails?.house_member[index]
                )
              );
            }
        ...
        const success = api.get(`${BACKEND_API_URL}job-draft`).then((res) => {
          const success = api
            .get(
              `${BACKEND_API_URL}agency/works-flow/?company=${templateDetails?.company}`
            )
            .then((res) => {
  ...
  const submitHandler = async (data) => {
    ...
    const create_job = await axios
      .put(`${BACKEND_API_URL}job-template/${templateId}/`, formData, config)
      .then((res) => {
        // console.log("EDITTTT", res.data);
        swal({
