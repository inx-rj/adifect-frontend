...
import { ADMIN_ACTIVITY_RESET } from "../../constants/activity-constants";
import { getAdminActivityDetails } from "../../redux/actions/activity-actions";
...
export default function Admin_Activity(props) {
  ...
  const {
    ...
    success: adminActivityDetailsSuccess,
    loading: adminActivityLoading,
  } = useSelector((state) => state.adminActivityReducer);
  ...
  useEffect(() => {
    // setTimeout(() => {
    dispatch({ type: ADMIN_ACTIVITY_RESET });
    ...
    if (filter) {
      dispatch(
        getAdminActivityDetails(props.agencyId, jobId, orderValue, filter)
      );
    } else {
      dispatch(getAdminActivityDetails(props.agencyId, jobId, orderValue));
    }
    ...
    dispatch(getMemberJobDetailsAction(jobId, props.agencyId));
    // }, 100);
  ...
  const handlePostChat = (e) => {
    ...
    formData.append("activity_job_chat[0]sender", props.agencyId);
  ...
  const handleSubmitRating = () => {
    ...
    formData.append("sender_user", props.agencyId);
      ...
      approveLoading ? (
        ...
        <>
          ...
          <div className="newContentActivityJob">
            {adminActivityDetails &&
              adminActivityDetails?.map((activity, index) => (
                <>
                  {/* ------------------------- JOB CREATE ------------------------- */}
                    ...
                    activity?.activity_status === 0 && (
                      <div className="newAgencyActivity">
                        <div className="MessImg">
                          ...
                          {adminActivityDetails?.length !== index + 1 && (
                      ...
                      <div className="ActivityBFourth">
                        {activity?.activity_job_chat[0]?.messages ? (
                          <div className="letsDotext">
                            ...
                            >
                              {urlify(activity?.activity_job_chat[0]?.messages)}
                            </p>
                          </div>
                        ) : (
                        ...
                        <div className="ActivityBFourth">
                          <div className="letsDotext">
                            <Rating
                              ...
                              value={activity?.job_rating_details?.rating}
                            />
                            <h5>{activity?.job_rating_details?.feedback}</h5>
                          </div>
                        </div>
                      </>
                    )} */}
