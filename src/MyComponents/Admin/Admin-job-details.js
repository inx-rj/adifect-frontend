...
// import Admin_Activity from "./Agency-activity";
import Admin_Activity from "./Admin-activity";
import Admin_job_detail_files from "./Admin-job-detail-files";
import Admin_job_detail_proposal from "./Admin-job-detail-proposal";
import Admin_job_detail_ques_ans from "./Admin-job-detail-ques-ans";
import Admin_job_details_new_page from "./Admin-job-details-new-page";
...
const Admin_job_details = () => {
  ...
  const {
    ...
    success,
    jobDetails,
  } = useSelector((state) => state.jobDetailsAgencyReducer);
  ...
  // console.log("getAdminJob", getAdminJob);
  ...
  useEffect(() => {
    dispatch(getJobDetailsAdmin(jobId));
    window.scrollTo(0, 0);
  }, [updateData]);
                  ...
                  <div className="Activitysec agencyjobdetailspage">
                    <Box sx={{ width: "100%" }}>
                      <TabContext value={value}>
                        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                          ...
                          >
                            ...
                            <Tab
                              ...
                              label={
                                <Badge
                                  className="badgeJobProposal"
                                  // showZero={true}
                                  badgeContent={not_seen_count}
                                  max={999}
                                  color="primary"
                                  sx={{
                                    "& .MuiBadge-badge": {
                        ...
                        <TabPanel value="1">
                          <Admin_job_details_new_page
                            setUpdateData={setUpdateData}
                          />
                        </TabPanel>
                        <TabPanel value="2">
                          <Admin_Activity
                            agencyId={getAdminJob.user}
                            setSubmitjobopen={setSubmitjobopen}
                            submitjobopen={submitjobopen}
                            jobName={getAdminJob.title}
                            jobCreatedAt={getAdminJob.created}
                            jobDueDate={getAdminJob.expected_delivery_date}
                            jobPrice={getAdminJob.price}
                            userid={getAdminJob.user}
                          />
                        </TabPanel>
                        <TabPanel value="3">
                          <Admin_job_detail_files userid={getAdminJob.user} />
                        </TabPanel>
                        <TabPanel value="4">
                          <Admin_job_detail_proposal
                            agencyId={getAdminJob.user}
                            jobDetails={getAdminJob}
                            jobProposalDate={getAdminJob.expected_delivery_date}
                            jobProposalPrice={getAdminJob.price}
                            handleChangeTab={handleChange}
                            statusJob={getAdminJob.is_active}
                            setUpdateData={setUpdateData}
                          />
                        </TabPanel>
                        <TabPanel value="5">
                          <Admin_job_detail_ques_ans
                            agencyId={getAdminJob.user}
                          />
                        </TabPanel>
                      </TabContext>
...
export default Admin_job_details;
