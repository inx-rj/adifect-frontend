...
import { AdminGetHelpUserListAction } from "../../redux/actions/Help-common-action";

export default function Admin_Help_Table() {
  ...
  const {
    ...
    error: adminError,
  } = useSelector((state) => state.AdminGetHelpUsersListReducer);
  ...
  useEffect(() => {
    dispatch(AdminGetHelpUserListAction());
  }, []);

  useEffect(() => {
    let userData = [];
    // console.log("adminUserListHelp ==== ", adminUserListHelp);
    adminUserListHelp?.map((item, index) => {
