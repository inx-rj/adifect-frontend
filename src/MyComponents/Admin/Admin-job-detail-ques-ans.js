...
const Admin_job_detail_ques_ans = (props) => {
  ...
  const { QuestionSearchData } = useSelector(
    (state) => state.SearchQuestionAdminReducer
  );
  ...
  const handleClickfiles = async (event) => {
    ...
    dispatch(
      listAllSearchAdmin({ order_by: orderby, status: demo, job_id: jobId })
    );
    sethidesearch(false);
  };
  ...
  const handlesearch = async (e) => {
    if (e.target.value) {
      dispatch(
        listAllSearchAdmin({ question: e.target.value, job_id: jobId })
      );
      sethidesearch(false);
    } else {
      sethidesearch(true);
...
export default Admin_job_detail_ques_ans;
