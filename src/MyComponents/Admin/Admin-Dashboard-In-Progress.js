...
import { SuperAdminJobListInProgressAction } from "../../redux/actions/job-actions";
import { ADMIN_DASHBOARD_IN_PROGRESS_JOBLIST_RESET } from "../../constants/job-constants";

function Admin_Dashboard_In_Progress() {
  ...
  const { loading: loadingSuperAdminJobList, SuperAdminJobList } = useSelector(
    (state) => state.SuperAdminInProgressListReducer
  );

  useEffect(() => {
    dispatch({ type: ADMIN_DASHBOARD_IN_PROGRESS_JOBLIST_RESET });

    dispatch(
      SuperAdminJobListInProgressAction({
        id: headerCompany ?? "",
        status: 2,
        page: 1,
    ...
    <>
      {loadingSuperAdminJobList ? (
        ...
        <div className="Work-In">
          ...
          {SuperAdminJobList?.slice(0, 5)?.map((item) => (
            <>
          ...
          {/* {SuperAdminJobList?.length > 5 && (
            <Link to={`/projects`} onClick={setLocalVarRedirect}>
              <span className="spanDashboardViewMore">View More...</span>
            </Link>
          )} */}
          {SuperAdminJobList?.length < 1 && (
...
export default Admin_Dashboard_In_Progress;
