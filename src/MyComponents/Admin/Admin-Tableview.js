...
const Admin_Media_datatable = () => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.checked) {
      if (name === "allSelect") {
        settotal(["Your Favorites", "Photo", "Videos", "Collections" , "Folders"]);
        setyourfilter(false);
      } else {
        settotal([...total, name]);
        setyourfilter(false);
 ...
 const [companyshow, setcompanyshow] = useState(true);
  ...
  const { success: moveon } = useSelector((state) => state.SuperAdminPostDamReducer);
  const { loading: moveonbro } = useSelector((state) => state.SuperAdminPostDamReducer);
  ...
  const { loading: rootimages } = useSelector((state) => state.SuperAdminDamRootImages);
  ...
  const { RootDamData } = useSelector((state) => state.SuperAdminRootDamReducer);

  const { DamImageData } = useSelector((state) => state.SuperAdminDamimageReducer);

  const { success: favpass } = useSelector((state) => state.SuperAdminDamimageReducer);

  const { DamFiltering } = useSelector((state) => state.SuperAdminDamFilterFavourate);
  const { DamFilterFavourateId } = useSelector(
    (state) => state.SuperAdminDamFilterFavourateID
  );

  const { DamDataImages } = useSelector((state) => state.SuperAdminDamIDIMAGESReducer);

  const { loading: checkof } = useSelector((state) => state.SuperAdminDamIDIMAGESReducer);
  ...
  const { DamRootImages } = useSelector((state) => state.SuperAdminDamRootImages);
...
settitleupdater(false)
  ...
  const { DamSearchnew } = useSelector((state) => state.SuperAdminDamSearchfolder);

  const { Dammostused } = useSelector((state) => state.SuperAdminlistAllMostUsed);

  const { DamParentFilter } = useSelector((state) => state.SuperAdminAllParentFilter);
...
if (jobshow) {
  ...
  const handleFileSubmit = () => {
    ...
    for (let index = 0; index < files.length; index++) {
      ...
      if (parentid) {
        ...
        axios
          .post(`${BACKEND_API_URL}super-admin-dam/?parent=${parentid}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
              Authorization: `Bearer ${userDataFromStore.token}`,
            },
  ...
  const movefilelocation = (value) => {
    ...
    if (collectiontrue && copydata) {
      ...
      dispatch(SuperAdminTitleupdate
        (
          {
      ...
      dispatch(SuperAdminDamMoveMultiple
      ({
          id: deletevalue,
          parent: moveparentfileid,
        })
  ...
  const renamedone = () => {
    dispatch(SuperAdminRenamefolder(
        {
          name: Renamechange,
          company:companyvalue,
        },
  ...
  const handleCollectionSubmit = () => {
    ...
    if (count.length > 0) {
      ...
      axios
        .post(`${BACKEND_API_URL}super-admin-dam/create_collection/`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${userDataFromStore.token}`,
          },
  ...
  const duplicateFile = () => {
    dispatch(SuperAdminDamcopypost({
        id: detailsid,
        parent: parentid,
        type: 3,
        type_new: 3,
        ...
        <>
          ...
          {isShown ? (
            ...
            <>
              ...
              <div class="Createmide">
                <div class="dammideanewdiv">
                {!damjobshow && (
                              <>
                  <div class="Createmidebtn">
                    <Link to="#">
                      <Button
                        id="basic-button"
                    ...
                    <DialogTitle className="profileImgfolder imgsizefixer imgsizefixermediafolder">
                      <h2>
                        ...
                        {!parentid && (
                          <>
                            {" "}
                            <Link to={`/admin-media-datatable`} className="closebuttonsec">
                              <i
                  ...
                  <div class="files_selected">
                    <div class="sharedampage">
                      {count.length > 0 && (
                        <>
                        ...
                        {inputRef.current?.length && (
                          <>
                            {inputRef.current
                             ...
                             {!CollectionNameShow && <> <p>({file.title.substring(0, 5)})</p></>}
                                      <ProgressBar
                                        now={file?.progress2}
                      ...
                      { RootDamData?.map((item) => (
                        <>
                              ...
                              >
                                ...
                                <h3 className="oneFiveNumber">{item.total_obj}</h3>
                            ...
                            {DamCountDataid?.collections?.map((item) => (
                             ...
                             >
                               <div className="yourFavDayDivcollection ">
                                 ...
                                 <h3 className="oneFiveNumber">{item.dam_media.length}</h3>
                                 <h3 className="oneSixNumber">
                               -
                              </h3>
                               </div>
