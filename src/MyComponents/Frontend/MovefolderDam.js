...
function Movemedia(props) {
  ...
  const { Dammovefolder } = useSelector((state) => state.damfoldermovereducer);
  const { DamROOTmovefolder } = useSelector(
    (state) => state.damrootfoldermovereducer
  );
  const { Damcollectioncopyfolder } = useSelector(
    (state) => state.damcollectionmovereducer
  );
  const { Damcollectioncopyfolder1 } = useSelector(
    (state) => state.damcollectionIDmovereducer
  );
  ...
  useEffect(() => {
    ...
    dispatch(DamrootMoveCollection());
    dispatch(DamMovefolderdata());
    dispatch(DamrootMovefolderdata());
  }, []);
  ...
  const copynavigate = (parent, is_parent, id, name) => {
    ...
    setAddNew([...addNew]);
    setParentidvalue(id);
    dispatch(DamMovefolderdata(id));
    dispatch(DamrootMoveCollectionID(id));
    setState([...state, name]);
  ...
  const handleClick2 = (event, key, element) => {
    ...
    if (parentidbreedcrumbs) {
      dispatch(DamMovefolderdata(parentidbreedcrumbs));
      dispatch(DamrootMoveCollectionID(parentidbreedcrumbs));
