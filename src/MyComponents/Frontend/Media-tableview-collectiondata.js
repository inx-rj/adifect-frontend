...
function TableViewAllcollection() {
  ...
  const getvalue1 = (e) => {
    ...
    if (e.target.value == null) {
      ...
      dispatch(DamCollectionSearch(collectionid,e.target.value));
  ...
  const favouritefile = () => {
    ...
    setTimeout(() => {
      dispatch(DamCollectioncount(collectionid));
    }, 500);
    setdetailsid()
  };

  const { CollectionDamCount } = useSelector(
    (state) => state.CollectionDamCount
  );
  ...
  const { CollectionDamSearch } = useSelector(
    (state) => state.CollectionDamSearch
  );
  ...
  const { CollectionDamFilter } = useSelector(
    (state) => state.CollectionDamFilter
  );
  ...
  const [total, settotal] = useState([]);
    ...
    dispatch(Collectionfilter(collectionid,params))
  ...
  const handleFileSubmit = () => {
    ...
    if (inputRef.current.length) {
      ...
      indexvalue.current = indexvalue.current + 1;
        ...
        axios
          .post(`${BACKEND_API_URL}agency/dam-media/`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
...
export default TableViewAllcollection;
