...
import CommentEditorTheme from "../../theme/PlaygroundEditorTheme";
...
export const INSERT_INLINE_COMMAND: LexicalCommand<string> = createCommand(
...
function useCollabAuthorName(): string {
  ...
  return yjsDocMap.has("comments") ? name : userData?.data?.username;
}
...
export default function CommentPlugin({
  ...
  const commentStore = useMemo(
    () => new CommentStore(editor, commentCacheStore),
    [editor, commentCacheStore]
  );
  const comments = useCommentStore(commentStore);

  useEffect(() => {
    if (comments) {
      setCommentCacheStore(comments);
    }
  }, [comments]);
