...
function useCollabAuthorName() {
  ...
  return yjsDocMap.has("comments") ? name : userData?.user?.name;
}

export default function CommentPlugin({
  providerFactory,
  commentCacheStore,setCommentCacheStore
}) {
  const collabContext = useCollaborationContext();
  const [editor] = useLexicalComposerContext();
  const commentStore = useMemo(() => new CommentStore(editor,commentCacheStore), [editor,commentCacheStore]);
