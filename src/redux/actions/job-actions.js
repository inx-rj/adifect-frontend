...
export const listAllJobs = (formData) => async (dispatch) => {
  try {
    ...
    if (companyId && companyId !== null && companyId !== "null") {
      ...
      data = await api.get(`${BACKEND_API_URL}jobs?page=${page}&${filter}&search=${search}`);
    }
...
export const FreshJobs = () => async (dispatch,getState) => {
  try {
    ...
    if (userData?.user?.user_level == 4) {
      data = await api.get(
        `${BACKEND_API_URL}members/member-latest-job/`,
      );
    } else {
      data = await api.get(
        `${BACKEND_API_URL}latest-job/`,
        config
      );
    }
...
export const getJobDetailsAdminNewPageAction = (id) => async (dispatch, getState) => {
  try {
    dispatch({
