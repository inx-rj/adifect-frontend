...
export const AllInvite = (companyid) => async (dispatch) => {
  try {
    dispatch({
    ...
    if (companyid) {
      data = await api.get(
        `${BACKEND_API_URL}agency/invite-member/?company=${companyid}`
      );
    } else {
      data = await api.get(`${BACKEND_API_URL}agency/invite-member/`);
    }
    dispatch({
...
export const invitedAdminMembersList =
  (companyid) => async (dispatch, getState) => {
    try {
      ...
      if (companyid) {
        data = await api.get(
          `${BACKEND_API_URL}agency/admin-members-list/?company=${companyid}`,
          config
        );
      } else {
        data = await api.get(
