...
const GET_TEMPLATES_LIST =
  (endpoint: string, filter: initialTableConfigInterface, id: number) =>
  async (dispatch: AppDispatch) => {
    dispatch(SET_TEMPLATES_DATA_LOADING(true));
    await TemplatesApiClient.fetchTemplatesList(endpoint, filter, id).then(
      (response) => {
        ...
        dispatch(SET_TEMPLATES_DATA(customizedResponse));
