...
import { SET_ALL_SKILLS_LIST_DATA, SET_ALL_SKILLS_LIST_LOADING, SET_SKILLS_LIST_LOADING, SET_USER_SKILL_SET_LIST_DATA, SET_USER_SKILL_SET_LIST_LOADING } from "redux/reducers/skills/skills.slice";
  ...
  () =>
    async (dispatch: AppDispatch) => {
      ...
      await skillsApiClient
        .getAllSkillsList()
        .then((response) => {
          console.log("All skill response", response);
          if (response.status === 201 || response.status === 200) {
