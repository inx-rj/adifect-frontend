...
const Audience = () => {
  ...
  useSingleEffect(() => {
    ...
    dispatch(GET_AUDIENCE_LIST({ ...tableFilters, ...filterData }));
  });
  ...
  const handleEdit = item => {
    ...
    const editableData = audienceList?.data?.results?.find(filterData => filterData?.id === item?.id);
    // @ts-ignore
                      ...
                      >
                        ...
                        >
                          <div className="grid grid-cols-2 w-full gap-x-3 gap-y-1">
                            ...
                            />
                            <h6 className="text-sm font-semibold mb-0">{channel_item?.channel_data?.name}</h6>
                            <div className="col-span-2 flex items-center gap-2">
                              <label className="text-sm text-gray-500">Title:</label>
                              <p className="text-sm text-gray-700">
                                {channel_item?.title ? channel_item?.title : 'NA'}
                              </p>
                            </div>
                            <div className=" flex items-center gap-2">
                              <label className="text-sm text-gray-500">Language:</label>
                              <p className="text-sm text-gray-700">
                                {channel_item?.language ? channel_item?.language : 'NA'}
                              </p>
                            </div>
                            <div className=" flex items-center gap-2">
                              <label className="text-sm text-gray-500">Device:</label>
                              <p className="text-sm text-gray-700">
                                {channel_item?.device ? channel_item?.device : 'NA'}
                              </p>
                            </div>
                            <div className=" flex items-center gap-2">
                              <label className="text-sm text-gray-500">Age:</label>
                              <p className="text-sm text-gray-700">{channel_item?.age ? channel_item?.age : 'NA'}</p>
                            </div>
                            <div className=" flex items-center gap-2">
                              <label className="text-sm text-gray-500">Gender:</label>
                              <p className="text-sm text-gray-700">
                                {channel_item?.gender ? channel_item?.gender : 'NA'}
                              </p>
                            </div>
                          </div>
                        </Card>
  ...
  const validateSubmit = e => {
    ...
    addAgencyFormData?.channel?.forEach(ch => {
      ...
      const chErr = {
        ...
        language: !channelSelect && (isEmpty(ch?.language) ? 'Language is required' : ''),
    ...
    const tempErrors = {
      ...
      audience_id: isEmpty(addAgencyFormData?.audience_id) ? 'Audience is required' : '',
      channel: channelArr
    };
      ...
      <div className="page-container">
        ...
        <div className="page-card new-card p-0">
          ...
          <MuiPopup
            ...
            textAlign="left"
            dialogContent={
              <AudienceAddModal formData={addAgencyFormData} setFormData={setAddAgencyFormData} errors={errors} />
            }
