...
import { Autocomplete, MenuItem, Select, TextField, createFilterOptions } from '@mui/material';
...
import { GET_INVITE_MEMBERS_USERS, GET_WORKFLOW_INVITE_USERS_LIST } from 'redux/actions/inviteUser/inviteUser.actions';
import {
...
import { WORKFLOW_MAIN_DETAILS, WORKFLOW_STAGE_DETAILS } from 'redux/reducers/workFlow/workFlow.slice';
...
const ApprovalWorkflow = () => {
  ...
  const handleOpenNudge = (index, nudge_index) => {
    const list = [...stages];
    list[index].nudgeOpen[nudge_index].open = !list[index].nudgeOpen[nudge_index].open;
    setStages(list);
    stagesRef.current = list;
  };
  ...
  const addMoreNudge = (e, index) => {
    ...
    if (list[index]?.nudge_time?.length < 4) {
      list[index].nudgeOpen = [...list[index].nudgeOpen, { id: prevLength + 1, open: false }];
      list[index].nudge_time = [...list[index].nudge_time, { id: prevLength + 1, time: notSelectedNudges[0] }];
    }
  ...
  const validateSubmit = e => {
    ...
    const tempErrors: any = {
      ...
      stages: stages.length <= 1 && !stages[0].stage_name && 'Atleast one workflow stage is required'
    };
  ...
  const submitHandler = async e => {
    ...
    const isError = Object.values(tempErrors).every(x => x === null || x === '');
    ...
    const isEmpty = Object.values(tempErrors).every(x => x === null || x === '');
      ...
      if (workflowStageDetails?.data?.results.length > 0) {
        for (let i in workflowStageDetails?.data?.results) {
          ...
          if (
            ...
            const nudgeTimeArr = workflowStageDetails?.data?.results[i]?.nudge_time?.split(',');
            nudgeTimeArr.pop();
          ...
          finalArr.push({
            ...
            is_all_approval: workflowStageDetails?.data?.results[i]?.is_all_approval,
            is_approval: workflowStageDetails?.data?.results[i]?.is_approval,
            approvals: workflowStageDetails?.data?.results[i]?.approvals_details,
            ...
            isDeadline: workflowStageDetails?.data?.results[i]?.approval_time ? true : false,
            ...
            nudgeOpen: finalNudgeOpenArr?.length > 0 ? finalNudgeOpenArr : [{ id: 1, open: false }],
            nudge_time: finalNudgeTimeArr?.length > 0 ? finalNudgeTimeArr : [{ id: 1, time: 3 }]
          });
        }
      } else {
        finalArr.push({
  ...
  const handleWorkFLowCompanyChange = (e, value) => {
    ...
    dispatch(GET_INVITE_MEMBERS_USERS(value?.id, 3));
    dispatch(GET_WORKFLOW_INVITE_USERS_LIST(value?.id));
  };
        ...
        <>
          ...
          {showbutton === true ? null : (
            <h6 className="currentlyWorkflow  bg-[#ffe2ad] rounded-t-lg h-[45px] gap-1 flex items-center justify-center text-[#cf5e00] text-base font-semibold">
              ...
              <span className="jobsusing font-bold border-b-2 border-[#cf5e00] text-base">Jobs using workflow</span>
            </h6>
          )}
          <div className="page-card p-7 AG-Workflow mt-0">
            <div className="border rounded-lg pt-7 px-7 mb-4 pb-2 grid md:grid-cols-2 sm:grid-cols-1 gap-5">
              <div className={errors.workflowName ? ' error' : ''}>
                <h5 className="text-base font-semibold">Approval Workflow Name</h5>
                <input
                  disabled={!showbutton}
              ...
              <div className={errors.companyvalue ? 'error  ' : ' '}>
                <div className="text-content ">
                  <h5 className="text-base font-semibold">Workflow Assign to Company?</h5>{' '}
                  <div className="">
                    <MuiAutoComplete
            ...
            <div id="items" className="gap-2">
              <DragDropContext onDragEnd={handleOnDragEnd}>
                ...
                <Droppable droppableId={'characters'}>
                  {provided => (
                    <ul {...provided.droppableProps} ref={provided.innerRef} className="characters">
                      {stagesRef?.current?.map((item, index) => {
                        // {stages?.map((item, index) => {
                        console.log('first', item.id, index, String(index), Droppable, stagesRef);
                        return (
                          <Draggable
                          ...
                          >
                            {provided => (
                              ...
                              >
                                ...
                                <div
                                  // key={index}
                                  className="border rounded-lg p-7 bg-white"
                                >
                                  <div className="Stage1 flex justify-between">
                                    ...
                                    <div className="Stage1_right_Workflow w-full max-w-[102px] flex gap-2 items-center">
                                      <div
                                        onClick={e => showbutton && orderUp(e, item.stage_id, index)}
                                      ...
                                      <div
                                        ...
                                        onClick={e => showbutton && orderDown(e, item.stage_id, index)}
                                        // onClick={(e) => setselected(index)}
                                      >
                                        {/* <img src="/img/don.png" /> */}
                                        <ArrowDownwardOutlined
                                      ...
                                      <div
                                        ...
                                        onClick={e => {
                                          showbutton &&
                                            (item.stage_id
                                              ? handleRemoveStage(index, item.stage_id, true)
                                              : handleRemoveStage(index, index, true));
                                        }}
                                      >
                                        <DeleteOutlineOutlined
                                          style={{
                                  ...
                                  <div className="Approvers">
                                    <h4 className="text-[#000] text-lg font-bold mt-5 mb-2.5">Approvers</h4>
                                    <div className="related-jobs-check-box flex items-center gap-2">
                                      <input
                                        className="checkbox"
                                        type="checkbox"
                                        // id="is_all_approval"
                                        onClick={e => showbutton && handleAllApproverChnge(e, index)}
                                      ...
                                      >
                                        {' '}
                                        All Approvers must approve job before it will move to the next stage
                                      </label>
                                    </div>
                                    <div className="related-jobs-check-box secondAllApprover flex items-center gap-2">
                                      <input
                                        ...
                                        type="checkbox"
                                        // id="is_approval"
                                        onClick={e => showbutton && handleApproverChnge(e, index)}
                                        // onChange={(e) => handleApproverChnge(e, index)}
                                        // setStages(...stages, { ...item, {item.!item.is_approval })
                                      ...
                                      >
                                        {' '}
                                        Approvers must approve job every time revisions are requested
                                      </label>
                                    </div>
                                    <div className="Observersskillssec mt-2.5 max-w-[586px] w-full">
                                      <div className="skills-input-container">
                                        <Autocomplete
                                          ...
                                          filterOptions={filterOptions}
                                          options={
                                            workflowMemberApproversData?.inviteMembersList?.data?.length > 0
                                              ? workflowMemberApproversData?.inviteMembersList?.data
                                              : []
                                          }
                                          getOptionLabel={option =>
                                    ...
                                    <div className="switch_Agency">
                                      <div className="Observers flex gap-3">
                                        ...
                                        <h4 className="text-[#000] text-lg font-bold">Observers</h4>
                                      </div>
                                    </div>
                                    <p className="Observerstext  text-base font-normal text-[#474e55] mt-2.5">
                                      Observers will see this stage of approvals, but will not be asked to approve
                                      anything.
                                    </p>
                                        ...
                                        >
                                          <div className="skills-input-container">
                                            <Autocomplete
                                              ...
                                              // }
                                              options={
                                                workflowMemberApproversData?.workflowInviteUserList?.data?.length > 0
                                                  ? workflowMemberApproversData?.workflowInviteUserList?.data
                                                  : []
                                              }
                                              getOptionLabel={option =>
                                                option?.user_first_name + ' ' + option?.user_last_name
                                              ...
                                              renderInput={params => (
                                                <TextField
                                                  ...
                                                  fullWidth
                                                  placeholder={
                                                    !companyvalue ? 'Select a company first' : 'Type something'
                                                  }
                                                />
                                              )}
                                              isOptionEqualToValue={
                                  ...
                                  >
                                    <input
                                      ...
                                      type="checkbox"
                                      // id="isDeadline"
                                      onClick={e => showbutton && handleIsDeadlineChnge(e, index)}
                                      // onChange={(e) => handleApproverChnge(e, index)}
                                      // setStages(...stages, { ...item, {item.!item.is_approval })
                                      defaultChecked={item.isDeadline}
                                      name="isDeadline"
                                    />
                                    <label className=" text-base font-normal text-[#474e55]" htmlFor="isDeadline">
                                      {' '}
                                    ...
                                    <>
                                      <div className="deadlineWorkflow">
                                        <h5 className="Deadlinetitle mt-5 mb-3 font-semibold text-base">
                                          ...
                                          Deadline for approval - Approver will have the assigned time below to approve
                                          asset once they receive the approval request
                                        </h5>
                                        <Select
                                          ...
                                          onChange={e => handleDeadlineChnge(e, index)}
                                          displayEmpty
                                          inputProps={{
                                            'aria-label': 'Without label'
                                          }}
                                      ...
                                      <div className="switch_Agency">
                                        <div className="Observers mt-4 flex gap-3">
                                          ...
                                          <h4 className="text-lg font-bold">Nudge</h4>
                                        </div>
                                      </div>
                                      {item?.is_nudge && (
                                        <>
                                          <div className="Observersskillssec">
                                            <div className="skills-input-container">
                                              ...
                                              {item?.nudge_time?.length &&
                                                item?.nudge_time?.map((nudge, nudge_index) => (
                                                  <>
                                                    <div className="selectAndRemoveRight">
                                                      <Select
                                                        ...
                                                        open={item.nudgeOpen[nudge_index].open}
                                                        onOpen={() => {
                                                          handleOpenNudge(index, nudge_index);
                                                        }}
                                                        onClose={() => {
                                                          handleOpenNudge(index, nudge_index);
                                                        }}
                                                        // MenuProps={menuProps}
                                                        value={nudge.time}
                                                        onChange={e => handleNudgeChnge(e, index, nudge_index)}
                                                        displayEmpty
                                                        inputProps={{
                                                          'aria-label': 'Without label'
                                                        }}
                                                      >
                                                        <MenuItem
                                                          hidden={item?.nudge_time?.some(el => el.time === 3)}
                                                          value={3}
                                                        >
                                                          3 hrs before the deadline
                                                        </MenuItem>
                                                        <MenuItem
                                                          hidden={item?.nudge_time?.some(el => el.time === 6)}
                                                          value={6}
                                                        >
                                                          6 hrs before the deadline
                                                        </MenuItem>
                                                        <MenuItem
                                                          hidden={item?.nudge_time?.some(el => el.time === 9)}
                                                          value={9}
                                                        >
                                                          9 hrs before the deadline
                                                        </MenuItem>
                                                        <MenuItem
                                                          hidden={
                                                            item?.nudge_time?.some(el => el.time === 12) ||
                                                            item.approval_time === 12
                                                          }
                                                          value={12}
                                                        >
                                                          12 hrs before the deadline
                                                        </MenuItem>
                                                      </Select>
                                                      <button
                                                        className="selectAndRemoveRightButton border border-[#2472FC] rounded-md"
                                                        hidden={nudge_index === 0}
                                                        onClick={e => removeNudge(e, index, nudge_index)}
                                                      >
                                          ...
                                          <button
                                            ...
                                            onClick={e => addMoreNudge(e, index)}
                                          >
                                            Add More
                                          </button>
                                        </>
            ...
            {errors.stages ? (
              <div
                ...
                className={errors.stages ? 'stageErrorWorkflow error' : 'stageErrorWorkflow notError'}
              >
                <span
                  className="text-[#D14F4F] flex justify-end"
                  style={{
            ...
            <div className="flex justify-between pt-5">
              ...
              >
                ...
                <button hidden={!showbutton} type="button" onClick={handleAddStage} className="btn btn-primary">
                  <Add />
                  Add Stage
                </button>
              </div>
