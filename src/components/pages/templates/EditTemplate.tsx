...
} from "redux/actions/templates/templates.actions";
...
const EditTemplate = () => {
      ...
      if (templateDetails?.details) {
        ...
        if (templateDetails?.details.is_house_member) {
          ...
          if (templateDetails?.details?.house_member?.length > 0) {
            ...
            for (
              let index = 0;
              index < templateDetails?.details?.house_member?.length;
              index++
            ) {
              houseMembersList.push(
                inHouseUserList?.inHouseUserList?.data?.results?.find(
                  (item) =>
                    item.id == templateDetails?.details?.house_member[index]
                )
              );
            }
        ...
        const success = axiosPrivate
          ...
          .then((res) => {
            if (userData?.data?.role === Roles.ADMIN) {
              axiosPrivate
                .get(
                  `${API_URL.WORKFLOW.ADMIN}?company=${templateDetails?.details?.company}`
                )
                .then((res) => {
                  dispatch(
                    SET_WORKFLOW_LIST_DATA(res?.data?.data || res?.data)
                  );
              ...
              const success = axiosPrivate
                .get(
                  `${API_URL.WORKFLOW.WORKFLOW_LIST}?company=${templateDetails?.details?.company}`
                )
                .then((response) => {
                  dispatch(
          ...
          for (var i = 0; i < templateDetails?.details?.images.length; i++) {
            fileext.push(
              templateDetails?.details?.images[i].job_images?.slice(
                ((templateDetails?.details?.images[i].job_images.lastIndexOf(
                  "."
                ) -
                  1) >>>
                  0) +
          ...
          for (var i = 0; i < templateDetails?.details?.images.length; i++) {
            fileext1.push(
              templateDetails?.details?.images[i].work_sample_images?.slice(
                ((templateDetails?.details?.images[
                  i
        ...
        if (
          isEmpty(
            responseDestructredData?.results?.find(
              (e) => e.company_id === templateDetails?.details?.company
            )
          )
        ) {
        ...
        if (
          isEmpty(
            destructuredWorkflowData?.results?.find(
              (e) => e?.id === templateDetails?.details?.workflow
            )
          )
        ) {
  ...
  useUpdateEffect(() => {
    ...
    companyData?.companyList?.data?.results,
    WorkFlowData?.workFlowList?.data?.results,
  ]);
  ...
  useEffect(() => {
    ...
    dispatch(GET_INDUSTRY_LIST(paginationData));
    dispatch(GET_COMPANY_LIST(paginationData));
  }, [addedSkill]);
  ...
  useEffect(() => {
    const success = axiosPrivate
      .get(`${API_URL.TEMPLATES.TEMPLATES_LIST}`)
      .then((res) => {
        settemplatevalue(res.data?.data?.results);
      });

    const tasks = axiosPrivate
      .get(`${API_URL.HOMEPAGE.JOB_TASK}`)
      .then((res) => {
        settasks(res?.data?.data?.results);
      });

    const industry = axiosPrivate
      .get(`${API_URL.INDUSTRY.INDUSTRY_LIST}`)
      .then((res) => {
  ...
  function removetaskdetailDocuments(document_id) {
    ...
    const success = axiosPrivate
      .delete(`${API_URL.HOMEPAGE.JOB_TASK}${document_id}/`)
      .then((res) => {
        // console.log(res);
      });
  }
  ...
  const getvalue = (e, value) => {
    ...
    if (value !== null) {
      ...
      if (userData?.data?.role === Roles.ADMIN) {
        axiosPrivate
          .get(`${API_URL.WORKFLOW.ADMIN}?company=${value?.company_id}`)
          .then((res) => {
        ...
        axiosPrivate
          .get(`${API_URL.WORKFLOW.WORKFLOW_LIST}?company=${value?.company_id}`)
          .then((res) => {
            // console.log(res.data);
            setapivalue(res.data?.data);
          });
  ...
  const gettemplate = (e) => {
    ...
    if (e.target.value == 1) {
      ...
      axiosPrivate
        .get(`${API_URL.TEMPLATES.TEMPLATES_LIST}${e.target.value}/`)
        .then((res) => {
          setlevel(res.data.level?.id);
          setJobType(res.data?.job_type);
          setPrice(res.data.price);
            ...
            <div className="w-full max-w-[560px]">
              ...
              >
                ...
                <p className="text-sm my-2 text-[#474e55]">
                  This could include dimensions, colors, how you plan to use it,
                  or anything else that the content creator needs to know in
                      ...
                      <div className="border-r w-full max-w-[35%] flex-center">
                        <div className="text-center p-5">
                          ...
                          >
                            ...
                            <p
                              className="text-base font-bold text-[#1B4EA8]"
                              style={{ color: "#1B4EA8" }}
                            >
