...
import { CREATOR_JOBS_DATA_IN_REVIEW } from "redux/reducers/jobs/creatorJobsList.slice";
...
const CreatorDashboardInReview = () => {
  ...
  const SuperAdminInReview = useAppSelector(CREATOR_JOBS_DATA_IN_REVIEW);
  ...
  useSingleEffect(() => {
    ...
    dispatch(GET_CREATORS_JOBLIST_IN_REVIEW(data));
  });
  ...
  return (
    <div>
      <div className="">
        ...
        {SuperAdminInReview?.data?.results?.slice(0, 5)?.map((item, index) => (
        ...
        {SuperAdminInReview?.data?.results?.length > 5 && (
        ...
        {SuperAdminInReview?.data?.results?.length < 1 && (
          <div className="mb-5">
            <div className="border-l-8 rounded border-[#D99836] bg-white p-6 h-full max-h-[580px] shadow-[0_4px_40px_#2472fc0f]">
