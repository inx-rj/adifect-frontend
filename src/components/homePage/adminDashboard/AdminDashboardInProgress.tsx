...
import { CLEAR_JOBS, JOBS_DATA } from '../../../redux/reducers/jobs/jobsList.slice';
    ...
    dispatch(
      GET_ADMIN_DASHBOARD_IN_PROGRESS_JOBLIST(
        userData?.data?.role === ROLES.MEMBER
          ...
          : userData?.data?.role === ROLES.AGENCY
  ...
  return (
    <div>
      ...
      <div className="pb-4 flex justify-between items-center">
        ...
        <div className="Sort">
          <h6 className="flex gap-2 items-center" style={{ cursor: 'pointer' }} onClick={handleClickInProgressSort}>
            <img src={Images.Sort} alt="" />
            <h5 className="text-sm font-semibold inline-block align-middle text-[#A0A0A0]">{orderingName}</h5>
          </h6>{' '}
          <Menu
            id="long-menu"
          ...
          >
            {menuOptions.map(option => (
              <MenuItem key={option.id} onClick={e => menuHandleSort(e, option.value, option.name)}>
                {option.name}
              </MenuItem>
            ))}
          </Menu>
      ...
      {SuperAdminJobList?.JobsListsList?.data?.results?.data?.slice(0, 5)?.map(item => (
        <>
          <Link to={`/jobs/details/${item.id}`}>
            <div className="mb-5">
              <div className="border-l-8 rounded border-[#2472FC] bg-white p-6 h-full max-h-[580px] shadow-[0_4px_40px_#2472fc0f]">
                <div className="pb-3">
                  <Title title={item?.title?.length > 30 ? `${item.title.slice(0, 30)}...` : item.title} />
                </div>
                ...
                <div className="mb-2 text-base font-semibold">Due to: {item?.job_due_date}</div>
                <div className="mb-2 flex gap-2 text-base font-semibold">
                  Assigned to: <h5 className="capitalize text-theme font-semibold">John snow</h5>
                </div>
      ...
      {SuperAdminJobList?.JobsListsList?.data?.results?.data?.length > 5 && (
        <Link to={`/projects`} onClick={setLocalVarRedirect}>
          <span className="spanDashboardViewMore">View More...</span>
        </Link>
      )}
      {SuperAdminJobList?.JobsListsList?.data?.results?.data?.length < 1 && (
        <div className="border-l-8 rounded border-[#2472FC] bg-white p-6 h-full max-h-[580px] shadow-[0_4px_40px_#2472fc0f]">
          <div className="text-lg font-semibold text-center">No jobs in progress</div>
        </div>
      )}
    </div>
  );
